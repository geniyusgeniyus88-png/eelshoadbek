...
from question.davlatlar import questions_davlat
from question.mantiq import questions_mantiq
from question.tarix import questions_tarix
from question.game import questions_game

from utils.skill import calculate_iq, detect_skills
from utils.poll_sender import send_quiz_poll

BOT_TOKEN = "8595761363:AAFGPKNUbMXJRsybAvji9r_Hcdfc_MMsDx0"
...
@dp.callback_query(F.data.in_(CATEGORY_DATA.keys()))
async def select_category(call: types.CallbackQuery):
    category = call.data

    selected = random.sample(CATEGORY_DATA[category], min(10, len(CATEGORY_DATA[category])))
...
async def send_next_question(user_id):
    ...
    if data["wrong"] >= 3 or data["index"] >= len(data["questions"]):
